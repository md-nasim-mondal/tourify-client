  ...
  return (
    <section className="py-16 md:py-24 bg-linear-to-b from-gray-50 to-white">
      <div className="container mx-auto px-4">
        ...
        <div className="mt-12 grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-6 mb-12">
          {categories.map((category) => (
            <Link
              key={category.name}
              href={`/explore?category=${encodeURIComponent(category.name)}`}
              className="group rounded-2xl bg-white p-6 text-center shadow-lg transition-all hover:shadow-xl hover:-translate-y-1 border border-gray-100"
            >
              <div className={`mx-auto flex h-16 w-16 items-center justify-center rounded-full ${category.color} text-white shadow-md`}>
                <category.icon className="h-8 w-8" />
              </div>
              <h3 className="mt-4 font-semibold text-gray-900 group-hover:text-primary transition-colors">{category.name}</h3>
