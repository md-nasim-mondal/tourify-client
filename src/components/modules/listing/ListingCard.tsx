...
export default function ListingCard({ listing }: ListingCardProps) {
  const img = listing.images && listing.images.length > 0 ? listing.images[0] : "/placeholder-tour.jpg";
  ...
  const rating = typeof listing.averageRating === "number" ? listing.averageRating.toFixed(1) : "0.0";
  const reviews = typeof listing.totalReviews === "number" ? listing.totalReviews : 0;
  const duration = listing.duration ? String(listing.duration) : "—";
  const maxGroupSize = typeof listing.maxGroupSize === "number" ? listing.maxGroupSize : undefined;
