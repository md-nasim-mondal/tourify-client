...
export default async function AdminListingsPage() {
  const res = await fetch(`${envVariables.BASE_API_URL}/listings?limit=20`, {
    cache: "no-store",
  });
  const json = await res.json();
  const listings: { id: string; title: string; location: string; status?: string }[] = json?.data || [];
      ...
      <div className="space-y-3">
        {listings.map((l) => (
          <div key={l.id} className="border rounded p-3 flex items-center justify-between">
            ...
            <form action={updateListingStatusAction.bind(null, l.id)} className="flex gap-2">
              <select name="status" defaultValue={l.status || "ACTIVE"} className="rounded border px-2 py-1">
