...
export default async function GuideListingsPage() {
  ...
  const res = await fetch(`${envVariables.BASE_API_URL}/listings/my-create`, {
    cache: "no-store",
      ...
      {listings && listings.length > 0 ? (
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
          {listings.map((it) => (
            <div
              key={it.id}
              className='border rounded-lg shadow-sm flex flex-col'>
              ...
              <div className='p-4 bg-gray-50 border-t flex items-center justify-end gap-2'>
                <Button asChild variant='outline' size='sm'>
                  <Link href={`/dashboard/guide/listings/edit/${it.id}`}>
                    Edit
                  </Link>
                </Button>
